 ...
 * @description Página de cadastro de produtos com suporte a grade (variações) e estoque sincronizado.
 */
export default function AdminAddProduct() {
    ...
    const [formData, setFormData] = useState({
        ...
        description: '', 
        price: '', 
        stock: 0, // Este valor é usado apenas se não houver variações
        brand_id: '', 
        category_id: '', 
        subcategory_id: '', 
        active: true
    ...
    // Cálculo dinâmico do estoque total da grade para exibição e salvamento
    ...
    const generateSKU = (index: number) => {
        ...
        const option = dbOptions.find(o => String(o.id) === String(row.variant_option_id));
        ...
        const valuePart = String(option.value).toUpperCase().replace(/\s/g, '');
        const randomPart = Math.floor(100 + Math.random() * 900);
        
        const newSku = `${namePart}-${valuePart}-${randomPart}`;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // 1. Validação inicial (Zod)
        const result = productSchema.safeParse(formData);
        if (!result.success) return toast.error(result.error.issues[0].message);
        if (!mainImage) return toast.error("A foto principal é obrigatória.");

        // 2. Validação da Grade (se ativa)
        if (hasVariations) {
            if (variations.length === 0) return toast.error("A grade está ativada, mas não há variações.");
            const invalidRow = variations.find(v => !v.variation_type_id || !v.variant_option_id || !v.sku);
            if (invalidRow) {
                return MySwal.fire({
                    title: 'Dados Incompletos',
                    text: 'Todas as variações precisam de Atributo, Valor e SKU.',
                    icon: 'error'
                });
            }
        }
        ...
        const finalConfirm = await MySwal.fire({
            ...
            text: `O estoque total será de ${hasVariations ? totalVariationStock : formData.stock} unidades.`,
            icon: 'question',
        ...
        setIsSaving(true);
        const loadingToast = toast.loading("Salvando produto e variações...");

        try {
            // 3. Montagem do FormData (Tabela Products)
            ...
            /**
             * REGRA DE NEGÓCIO: Sincronização de Estoque
             * Se houver variações, o estoque principal é a SOMA delas.
             * Isso garante que o produto NÃO apareça como esgotado no front.
             */
            const stockToSend = hasVariations ? totalVariationStock : formData.stock;
            data.append('stock', String(stockToSend));

            // Imagens
            if (mainImage) data.append('image_1', mainImage);
            gallery.forEach((file, i) => { if (i < 5) data.append(`image_${i + 2}`, file); });

            // 4. Criação do Produto Pai
            ...
            // 5. Criação das Variações Filho (Se houver)
            ...
            toast.error(error.response?.data?.message || "Erro ao salvar", { id: loadingToast });
        } finally { 
            setIsSaving(false); 
        }
    };
    ...
    return (
        <div className="min-h-screen bg-[#F8F9FB] p-4 md:p-8 font-sans text-zinc-900 italic leading-none">
            <Toaster position="top-center" />
            
            {/* Header Fixo/Sticky seria ideal aqui, mas mantendo seu layout */}
            <header className="max-w-6xl mx-auto mb-6 flex items-center justify-between">
                ...
                <button 
                    ...
                    type="submit" 
                    disabled={isSaving} 
                    className="bg-zinc-900 text-white px-6 py-3 rounded-xl font-black uppercase italic text-xs flex items-center gap-2 hover:bg-yellow-600 transition-all disabled:opacity-50"
                >
                    {isSaving ? "Processando..." : <><Save size={16} /> Publicar Produto</>}
                </button>
            </header>
            ...
            <form id="product-form" onSubmit={handleSubmit} className="max-w-6xl mx-auto grid lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-white p-5 rounded-3xl border border-zinc-200 shadow-sm">
                        <h4 className="text-[10px] font-black uppercase text-zinc-400 mb-4 tracking-widest flex items-center gap-2">
                            <ImageIcon size={14} /> Foto Principal
                        </h4>
                        <label className="group relative aspect-[3/4] bg-zinc-50 rounded-2xl border-2 border-dashed border-zinc-200 flex flex-col items-center justify-center cursor-pointer overflow-hidden hover:border-yellow-500 transition-all">
                            {mainImage ? (
                                ...
                                <div className="text-center text-zinc-300">
                                    <Upload size={32} />
                                    <span className="text-[10px] font-bold uppercase mt-2 block">Upload Foto</span>
                                </div>
                            )}
                    ...
                    <div className="bg-white p-5 rounded-3xl border border-zinc-200 shadow-sm">
                        <h4 className="text-[10px] font-black uppercase text-zinc-400 mb-4 tracking-widest flex items-center gap-2">
                            <Layers size={14} /> Galeria (Até 5 fotos)
                        </h4>
                        <div className="grid grid-cols-3 gap-2">
                            ...
                            {gallery.length < 5 && (
                                <label className="aspect-square bg-zinc-50 border-2 border-dashed border-zinc-200 rounded-lg flex items-center justify-center text-zinc-300 cursor-pointer hover:border-yellow-500 transition-all">
                                    <Plus size={20} />
                                    <input type="file" multiple className="hidden" accept="image/*" onChange={e => setGallery([...gallery, ...Array.from(e.target.files || [])].slice(0, 5))} />
                                </label>
                            )}
                        </div>
                    </div>
                ...
                {/* Coluna Principal: Detalhes e Grade */}
                <div className="lg:col-span-8 space-y-6">
                    <div className="bg-white p-6 md:p-8 rounded-3xl border border-zinc-200 shadow-sm space-y-8">
                        <div className="grid md:grid-cols-2 gap-5 text-left">
                            <div className="md:col-span-2">
                                <label className="text-[10px] font-black uppercase text-zinc-400 ml-2 tracking-wider">Nome Comercial</label>
                                <input 
                                    required 
                                    placeholder="Ex: Tênis Adidas Forum Low" 
                            ...
                            />
                            <div>
                                <label className="text-[10px] font-black uppercase text-zinc-400 ml-2 tracking-wider">Preço Base (R$)</label>
                                <input 
                                    type="number" 
                                    step="0.01" 
                                    className="w-full bg-zinc-50 border border-zinc-100 p-4 rounded-xl font-black text-lg outline-none focus:border-yellow-500 text-zinc-900" 
                            ...
                            <SelectSearch 
                                ...
                                onChange={id => setFormData({ ...formData, category_id: id, subcategory_id: '' })} 
                            />
                            <SelectSearch 
                                label="Subcategoria" 
                        ...
                        {/* SEÇÃO DE GRADE / VARIAÇÕES */}
                        <div className="pt-6 border-t border-zinc-100">
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex items-center gap-2">
                                    <ListTree size={18} className="text-yellow-600" />
                                    <h3 className="font-black text-xs uppercase tracking-widest">Gestão de Estoque e Variações</h3>
                                </div>
                                <button 
                                    type="button" 
                                    onClick={() => setHasVariations(!hasVariations)} 
                                    className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all shadow-sm ${hasVariations ? 'bg-red-50 text-red-500' : 'bg-zinc-900 text-white hover:bg-yellow-600'}`}
                                >
                                    {hasVariations ? 'Remover Grade' : 'Ativar Grade (Cor/Tamanho)'}
                                </button>
                            </div>

                            {hasVariations ? (
                                <div className="border border-zinc-200 rounded-2xl bg-white overflow-visible">
                                    <table className="w-full text-left border-collapse" style={{ tableLayout: 'fixed' }}>
                                        <thead>
                                            <tr className="bg-zinc-50 border-b border-zinc-200">
                                                ...
                                                <th className="p-4 w-24 text-[9px] font-black uppercase text-zinc-400 tracking-widest text-center">Qtd</th>
                                                <th className="p-4 w-12 text-center"></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {variations.map((v, i) => (
                                                <tr key={i} className="border-b border-zinc-50 last:border-0 relative" style={{ zIndex: variations.length - i }}>
                                                    <td className="p-2 overflow-visible">
                                                        <SelectSearch 
                                                            label="" 
                                                            options={variationTypeOptions} 
                                                            value={v.variation_type_id} 
                                                            onChange={id => updateVariation(i, 'variation_type_id', id)} 
                                                        />
                                                    </td>
                                                    <td className="p-2 overflow-visible">
                                                        <SelectSearch
                                                            label=""
                                                            options={dbOptions
                                                                .filter(o => Number(o.variation_type_id) === Number(v.variation_type_id))
                                                                .map(o => ({ value: String(o.id), label: String(o.value) }))
                                                            }
                                                    ...
                                                    <td className="p-2">
                                                        <div className="relative group">
                                                            <input 
                                                                placeholder="Ex: TENIS-42" 
                                                                className="w-full bg-white border border-zinc-200 p-3 pr-10 rounded-xl text-[10px] font-mono font-bold uppercase outline-none focus:border-yellow-500 text-zinc-900" 
                                                                value={v.sku} 
                                                            ...
                                                            <button 
                                                                ...
                                                                className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-300 hover:text-yellow-600 transition-colors"
                                                            >
                                                                <Wand2 size={14} />
                                                            </button>
                                                        </div>
                                                    ...
                                                    <td className="p-2">
                                                        <input 
                                                            type="number" 
                                                            className={`w-full p-3 rounded-xl text-xs font-black text-center outline-none focus:ring-2 ring-yellow-500 ${v.stock <= 0 ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-zinc-900 text-white'}`} 
                                                            value={v.stock} 
                                        ...
                                        {/* Rodapé com Total de Estoque para Clareza */}
                                        {variations.length > 0 && (
                                            <tfoot>
                                                <tr className="bg-zinc-50/50">
                                                    <td colSpan={3} className="p-4 text-right text-[10px] font-black uppercase text-zinc-400">Total calculado:</td>
                                                    <td className="p-4 text-center text-xs font-black text-yellow-600">{totalVariationStock} un</td>
                                                    <td></td>
                                                </tr>
                                            </tfoot>
                                        )}
                                    ...
                                    <button 
                                        ...
                                        className="w-full p-4 bg-zinc-50 text-[10px] font-black uppercase text-zinc-400 hover:bg-yellow-50 hover:text-yellow-600 transition-all border-t flex items-center justify-center gap-2"
                                    >
                                        <Plus size={14} /> Adicionar Linha à Grade
                                    </button>
                                </div>
                            ) : (
                                <div className="bg-zinc-50 border border-zinc-100 rounded-3xl p-8 flex flex-col items-center justify-center">
                                    <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-4">Estoque Central Único</span>
                                    <input 
                                        type="number" 
                                        value={formData.stock} 
                                        placeholder="Definir Qtd" 
                                        className="max-w-[180px] w-full bg-white border border-zinc-200 p-4 rounded-xl font-black text-center text-lg outline-none focus:border-yellow-500 shadow-sm text-zinc-900" 
                        ...
                        <div className="pt-6 border-t border-zinc-100 text-left">
                            ...
                            <textarea 
                                ...
                                rows={4} 
                                placeholder="Descreva os diferenciais, materiais e cuidados com o produto..." 
                                className="w-full bg-zinc-50 border border-zinc-100 p-5 rounded-2xl font-bold italic outline-none focus:bg-white focus:border-yellow-500 text-zinc-900" 

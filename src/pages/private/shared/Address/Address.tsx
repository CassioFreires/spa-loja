...
export default function CheckoutAddress() {
    ...
    const loadAddresses = async () => {
        ...
        try {
            ...
            // Se for 404, o usuário apenas não tem endereços ainda - comportamento normal
    ...
    if (loading) return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-[#F8F9FB]">
            ...
            <span className="font-black uppercase italic text-[10px] tracking-widest text-zinc-400">Verificando endereços...</span>
        </div>
    );
            ...
            <main className="max-w-4xl mx-auto space-y-6">
                <div className="grid gap-4">
                    {addresses.length > 0 ? (
                        ...
                        <div className="bg-white border-2 border-dashed border-zinc-200 rounded-[3rem] p-16 text-center animate-in fade-in zoom-in-95 duration-500">
                            <MessageSquare size={40} className="text-zinc-200 mx-auto mb-4" />
                            <h3 className="text-xl font-black uppercase italic mb-2 tracking-tighter">Nenhum local <span className="text-yellow-600">cadastrado</span></h3>
                            <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-8">Adicione um endereço para prosseguir com a entrega.</p>
                            <button onClick={() => handleOpenModal()} className="bg-zinc-900 text-white px-8 py-4 rounded-2xl font-black uppercase italic text-xs hover:bg-yellow-600 transition-all">Cadastrar Novo</button>
                        </div>
                    )}
                </div>

                {/* PAINEL DE PROSSEGUIR */}
                {selectedAddressId && (
                    <div className="mt-12 bg-white p-8 rounded-[2.5rem] border border-zinc-200 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6 animate-in slide-in-from-bottom-4 duration-500">
                        <div className="flex items-center gap-6 text-left">
                            ...
                            <div>
                                <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-1 italic">Tudo pronto!</p>
                                <h4 className="font-black text-lg italic uppercase leading-none text-zinc-900">Endereço de envio definido</h4>
                                <p className="text-[10px] font-bold text-zinc-500 uppercase mt-1 leading-none">Clique abaixo para escolher a forma de pagamento</p>
                            </div>
                        </div>

        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-2 md:p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-4xl max-h-[95vh] overflow-y-auto rounded-[2rem] md:rounded-[3rem] shadow-2xl relative flex flex-col italic text-zinc-900 border border-zinc-200 no-scrollbar">
                ...
                <div className="p-6 md:p-12 space-y-10 md:space-y-12">
                    ...
                    {/* STATUS DE ENTREGA / TIMELINE HORIZONTAL */}
                            ...
                            {events.length > 0 ? (
                                ...
                                [...events].reverse().map((event: any, idx: number) => (
                                    <div key={idx} className="relative flex gap-6 pl-12 group animate-in slide-in-from-left-4 duration-500">
                                        ...
                                        `}>
                                            {idx === 0 ? (
                                                // Se estiver entregue, mostra Check. Se não, mostra Loading.
                                                order.status === 'ENTREGUE' ? <CheckCircle2 size={16} /> : <Loader2 size={16} className="animate-spin" />
                                            ) : (
                                ...
                                <div className="pl-12 py-4 text-left">
                                    <p className="text-[10px] font-bold text-zinc-300 uppercase tracking-widest italic">Aguardando atualizações do percurso...</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 border-t border-zinc-50 pt-10">
                        <div className="space-y-3">
                            ...
                            <div className="text-left text-sm leading-relaxed"><p className="font-black text-zinc-800 uppercase">{customerName}</p><p className="text-zinc-500 font-bold break-all">{customerEmail}</p></div>
                        </div>
                        <div className="space-y-3">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="text-left">
                                <div className="mb-3">
                                    <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase border ${order.status === 'PAGO' ? 'bg-green-50 text-green-600 border-green-100' : order.status === 'CANCELADO' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-yellow-50 text-yellow-600 border-yellow-100'}`}>{order.status}</span>
                                </div>
                                {order.tracking_code && <div className="flex items-center gap-2 bg-zinc-900 text-yellow-500 px-2 py-1 rounded-lg w-fit"><Hash size={12} /><span className="text-[11px] font-black uppercase">{order.tracking_code}</span></div>}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {order.items?.map((item: any, idx: number) => (
                                <div key={idx} className="flex items-center justify-between bg-zinc-50/50 p-4 rounded-3xl border border-zinc-100 group hover:bg-zinc-50 transition-all">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="text-left"><h5 className="font-black uppercase text-[10px] text-zinc-900 line-clamp-1">{item.product_name}</h5><p className="text-[9px] font-bold text-zinc-400 uppercase mt-1">Qtd: {item.quantity}</p></div>
                                    </div>
                    ...
                    <div className="bg-zinc-900 text-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 relative overflow-hidden shadow-xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                            <div className="order-2 md:order-1">
                                {order.status === 'PENDENTE' ? (
                                    <div className="flex flex-col gap-3">
                                        ...
                                        {order.payment_url && (
                                            <button onClick={() => window.open(order.payment_url, '_blank')} className="bg-yellow-500 text-black w-full py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-white transition-all shadow-lg active:scale-95"><ExternalLink size={14} /> Pagar Agora</button>
                                        )}
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center"><CreditCard size={16} className="text-yellow-500" /></div>
                                        <div className="text-left"><p className="text-[9px] font-medium text-zinc-400 uppercase tracking-widest mb-1">Meio de Captura</p><p className="text-sm font-black uppercase tracking-tight">{order.capture_method || 'InfinitePay'}</p></div>
                                    </div>
                                )}
                            </div>
                            <div className="order-1 md:order-2 space-y-3">
                                ...
                                <div className="flex justify-between text-[10px] font-black uppercase text-zinc-400 tracking-[0.1em]"><span>Frete</span><span className="text-white">R$ {Number(order.amount_paid_shipping || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span></div>
                                <div className="pt-4 border-t border-white/10 flex justify-between items-end"><span className="font-black uppercase italic text-lg leading-none">Total</span><div className="text-right leading-none"><span className="text-3xl md:text-4xl font-black text-white italic tracking-tighter leading-none">R$ {Number(order.total_value).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span></div></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 md:p-8 bg-zinc-50 border-t border-zinc-100 flex flex-col md:flex-row gap-6 items-center justify-between mt-auto">
                    <div className="flex items-center gap-2 text-zinc-400">
                        <FileText size={16} />
                        <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-center md:text-left leading-tight">Certificado Gold • {new Date(order.created_at).toLocaleDateString('pt-BR')} • {new Date(order.created_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>

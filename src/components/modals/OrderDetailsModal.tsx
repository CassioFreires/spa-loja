...
export default function OrderDetailModal({ order, isOpen, onClose, isAdmin = false }: OrderDetailModalProps) {
    ...
    // AJUSTE: staleTime: 0 e refetch garantem que os dados do Postman apareçam na hora
    ...
    const mutationCancel = useMutation({
        mutationFn: async () => {
            ...
            if (token) return cancelOrder(order.id);
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-2 md:p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-4xl max-h-[95vh] overflow-y-auto rounded-[2rem] md:rounded-[3rem] shadow-2xl relative flex flex-col italic text-zinc-900 border border-zinc-200 no-scrollbar">
                ...
                <div className="p-6 md:p-12 space-y-10 md:space-y-12">
                    ...
                    {/* JORNADA DETALHADA (LISTA VERTICAL AJUSTADA) */}
                    <div className="space-y-8">
                        ...
                        <div className="relative space-y-8 before:absolute before:left-[17px] before:top-2 before:bottom-2 before:w-[2px] before:bg-zinc-100 ml-2">
                            {events.length > 0 ? (
                                // Invertemos para que o evento MAIS RECENTE seja o PRIMEIRO (idx === 0)
                                ...
                                <div className="pl-12 py-4 text-left font-bold text-[10px] text-zinc-300 uppercase tracking-widest italic">
                                    Processando jornada logística...
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 border-t border-zinc-50 pt-10 text-left">
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center gap-2 text-zinc-400"><MapPin size={14} /><span className="text-[10px] font-black uppercase tracking-widest">Entrega</span></div>
                            <div>
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase border mb-3 inline-block ${order.status === 'PAGO' ? 'bg-green-50 text-green-600 border-green-100' : order.status === 'CANCELADO' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-yellow-50 text-yellow-600 border-yellow-100'}`}>{order.status}</span>
                    ...
                    <div className="bg-zinc-900 text-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 relative overflow-hidden shadow-xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                            <div className="order-2 md:order-1 text-left">
                                {order.status === 'PENDENTE' ? (
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div><p className="text-[9px] font-medium text-zinc-400 uppercase tracking-widest mb-1">Meio de Captura</p><p className="text-sm font-black uppercase tracking-tight">{order.capture_method || 'InfinitePay'}</p></div>
                                    </div>
                                )}
                            </div>
                            <div className="order-1 md:order-2 space-y-3">
                                ...
                                <div className="flex justify-between text-[10px] font-black uppercase text-zinc-400 tracking-[0.1em]"><span>Frete Loggi</span><span className="text-white">R$ {Number(order.amount_paid_shipping || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span></div>
